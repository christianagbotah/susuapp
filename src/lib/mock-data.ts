...
export const mobileMoneyProviders: MobileMoneyProvider[] = [
  ...
  { id: 'telecel', name: 'Telecel Cash', shortName: 'Telecel', code: 'TCL', color: '#E60000', bgColor: '#FFEBEE', icon: '📲', ussdCode: '*110#' },
  { id: 'at', name: 'AT Money', shortName: 'AT Money', code: 'AT', color: '#005BAC', bgColor: '#E3F2FD', icon: '📲', ussdCode: '*500#' },
];
...
export const loanProducts: LoanProduct[] = [
  ...
  { id: 'lp-006', name: 'Instant Micro Loan', description: 'Get instant cash via mobile money in under 2 minutes. No guarantor, no paperwork. Repay daily or weekly.', type: 'personal', minAmount: 50, maxAmount: 2000, interestRate: 5, minTerm: 7, maxTerm: 30, termUnit: 'days', requirements: ['Active account 30+ days', 'At least 3 susu contributions', 'Basic KYC (Ghana Card)'], isActive: true, processingFeePercent: 2, repaymentFrequency: 'daily', interestType: 'flat', autoApprove: true, maxCreditScoreRequired: 40 },
];
...
export const loans: Loan[] = [
  ...
  { id: 'ln-002', applicantId: 'usr-001', applicantName: 'Ama Mensah', applicantPhone: '0241234567', type: 'education', amount: 5000, interestRate: 6, term: 6, termUnit: 'months', status: 'repaid', monthlyPayment: 860, remainingBalance: 0, totalPaid: 5160, nextPaymentDate: '', startDate: '2025-06-01', endDate: '2025-12-01', disbursementMethod: 'momo', disbursementProvider: 'Telecel Cash', disbursementNumber: '0241234567', purpose: 'Children school fees payment', guarantorName: 'Efua Darko', guarantorPhone: '0501112233', creditScore: 78, branch: 'Accra Central' },
  ...
  // Fido-style Instant Micro Loans
  { id: 'ln-009', applicantId: 'usr-001', applicantName: 'Ama Mensah', applicantPhone: '0241234567', type: 'personal', productId: 'lp-006', amount: 500, interestRate: 5, term: 10, termUnit: 'days', status: 'active', monthlyPayment: 52.5, remainingBalance: 447.5, totalPaid: 52.5, nextPaymentDate: '2026-04-19', startDate: '2026-04-09', endDate: '2026-04-19', disbursementMethod: 'momo', disbursementProvider: 'MTN Mobile Money', disbursementNumber: '0241234567', purpose: 'Restock provisions at Makola', creditScore: 78, branch: 'Accra Central', disbursementDate: '2026-04-09', applicationDate: '2026-04-09', approvedDate: '2026-04-09', autoApproved: true, repaymentFrequency: 'daily', processingFee: 10, totalInterest: 25, interestType: 'flat', repaymentSchedule: (() => { const s: RepaymentScheduleEntry[] = []; const base = new Date('2026-04-09'); const totalInt = 500 * 0.05; for (let i = 0; i < 10; i++) { const d = new Date(base); d.setDate(d.getDate() + i + 1); const isLast = i === 9; s.push({ dueDate: d.toISOString().split('T')[0], amount: isLast ? Math.round((500 + totalInt - 52.5 * i) * 100) / 100 : 52.5, principal: isLast ? Math.round((500 - 50 * i) * 100) / 100 : 50, interest: isLast ? Math.round((totalInt - 2.5 * i) * 100) / 100 : 2.5, status: i < 1 ? 'paid' : 'pending', paidDate: i < 1 ? '2026-04-10' : undefined, paidAmount: i < 1 ? 52.5 : undefined }); } return s; })() },
  { id: 'ln-010', applicantId: 'usr-002', applicantName: 'Kwame Asante', applicantPhone: '0209876543', type: 'personal', productId: 'lp-006', amount: 300, interestRate: 5, term: 14, termUnit: 'days', status: 'active', monthlyPayment: 21.07, remainingBalance: 278.93, totalPaid: 21.07, nextPaymentDate: '2026-04-19', startDate: '2026-04-05', endDate: '2026-04-19', disbursementMethod: 'momo', disbursementProvider: 'MTN Mobile Money', disbursementNumber: '0209876543', purpose: 'Emergency transport fare', creditScore: 65, branch: 'Kumasi Adum', disbursementDate: '2026-04-05', applicationDate: '2026-04-05', approvedDate: '2026-04-05', autoApproved: true, repaymentFrequency: 'daily', processingFee: 6, totalInterest: 15, interestType: 'flat', repaymentSchedule: (() => { const s: RepaymentScheduleEntry[] = []; const base = new Date('2026-04-05'); const totalInt = 300 * 0.05; for (let i = 0; i < 14; i++) { const d = new Date(base); d.setDate(d.getDate() + i + 1); const isLast = i === 13; s.push({ dueDate: d.toISOString().split('T')[0], amount: isLast ? Math.round((300 + totalInt - 22.5 * i) * 100) / 100 : 22.5, principal: isLast ? Math.round((300 - (300/14) * i) * 100) / 100 : Math.round((300/14) * 100) / 100, interest: isLast ? Math.round((totalInt - (totalInt/14) * i) * 100) / 100 : Math.round((totalInt/14) * 100) / 100, status: i < 1 ? 'paid' : i === 2 ? 'overdue' : 'pending', paidDate: i < 1 ? '2026-04-06' : undefined, paidAmount: i < 1 ? 22.5 : undefined, overdueDays: i === 2 ? 1 : undefined }); } return s; })() },
  { id: 'ln-011', applicantId: 'usr-003', applicantName: 'Efua Darko', applicantPhone: '0501112233', type: 'personal', productId: 'lp-006', amount: 800, interestRate: 5, term: 7, termUnit: 'days', status: 'active', monthlyPayment: 118.57, remainingBalance: 783.43, totalPaid: 16.57, nextPaymentDate: '2026-04-19', startDate: '2026-04-12', endDate: '2026-04-19', disbursementMethod: 'momo', disbursementProvider: 'Vodafone Cash', disbursementNumber: '0501112233', purpose: 'Catering supplies for event', creditScore: 82, branch: 'Takoradi', disbursementDate: '2026-04-12', applicationDate: '2026-04-12', approvedDate: '2026-04-12', autoApproved: true, repaymentFrequency: 'daily', processingFee: 16, totalInterest: 40, interestType: 'flat', repaymentSchedule: (() => { const s: RepaymentScheduleEntry[] = []; const base = new Date('2026-04-12'); const totalInt = 800 * 0.05; for (let i = 0; i < 7; i++) { const d = new Date(base); d.setDate(d.getDate() + i + 1); const isLast = i === 6; s.push({ dueDate: d.toISOString().split('T')[0], amount: isLast ? Math.round((800 + totalInt - 120 * i) * 100) / 100 : 120, principal: isLast ? Math.round((800 - (800/7) * i) * 100) / 100 : Math.round((800/7) * 100) / 100, interest: isLast ? Math.round((totalInt - (totalInt/7) * i) * 100) / 100 : Math.round((totalInt/7) * 100) / 100, status: 'pending' }); } return s; })() },
];
...
export const transactions: Transaction[] = [
  ...
  { id: 'txn-008', userId: 'usr-001', type: 'withdrawal', amount: 500, currency: 'GHS', status: 'completed', date: '2026-04-13T15:30:00', description: 'Withdrawal to Telecel Cash', reference: 'SP-W001-V4W5X6', category: 'Wallet', balanceAfter: 5900, counterpartName: 'Telecel Cash' },
  { id: 'txn-009', userId: 'usr-001', type: 'deposit', amount: 1000, currency: 'GHS', status: 'completed', date: '2026-04-12T08:45:00', description: 'Deposit via AT Money', reference: 'SP-D002-Y7Z8A9', category: 'Wallet', balanceAfter: 6400, counterpartName: 'AT Money' },
...
export const disputes: Dispute[] = [
  { id: 'dsp-001', transactionId: 'txn-008', userId: 'usr-001', type: 'incorrect_amount', description: 'I withdrew ₵500 to Telecel Cash but only ₵300 was received in my wallet.', amount: 500, status: 'open', date: '2026-04-13T16:00:00', updatedAt: '2026-04-13T16:00:00' },
